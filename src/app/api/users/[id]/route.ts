...
export async function PATCH(request: Request, ctx: RouteContext<"/api/users/[id]">) {
  ...
  return handle(request, patchSchema, async ({ body, context }) => {
    ...
    await recordAudit(context, {
      ...
      changes: {
        before: { status: user.status, role: user.role.name, units: user.unitAccess.length },
        after: { status: updated.status, role: roleName, units: body.unitIds?.length ?? user.unitAccess.length },
      },
    });
